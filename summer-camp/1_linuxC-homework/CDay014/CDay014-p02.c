...
bool isValidBst(TreeNode* root){
    ...
    if(root->left != NULL && max(root->left) >= root->key) return false;
    if(root->right != NULL && min(root->right) <= root->key) return false;
    if(!isValidBst(root->left) || !isValidBst(root->key)) return false;
...
int main(){
   ...
   printf("valid: %d ", isValidBst(tree->root));
   return 0;
}
